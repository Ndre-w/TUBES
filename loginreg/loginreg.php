...
$pesan_error_login = '';
$registration_error = false; // Variable to track registration error
        ...
        if ($_POST['form_type'] == 'registration') {
            ...
            if ($result->num_rows > 0) {
                ...
                if ($koneksi->query($sql) === TRUE) {
                    // Registration successful, set a message to be displayed on the form
                    $pesan_success_registrasi = "Registration successful! Please proceed to sign in.";
                } else {
                    $registration_error = true;
                    $pesan_error_registrasi = "Error: " . $sql . "<br>" . $koneksi->error;
                }
            }
            ...
            if (!empty($email) && !empty($kata_sandi)) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    if (password_verify($kata_sandi, $row['kata_sandi'])) {
                        ...
                        if ($row['level'] == 1) {
                            // Pengguna level 1 (user), redirect ke halaman user.php atau halaman lainnya
                            header("Location: ../users/users.php");
                            exit();
                        } elseif ($row['level'] == 2) {
                            // Pengguna level 2 (tim pengontrol), redirect ke halaman tim_pengontrol.php atau halaman lainnya
                            header("Location: ../pengontrol/kamerastok.php");
                            exit();
                        } elseif ($row['level'] == 3) {
                            // Pengguna level 3 (admin), redirect ke halaman admin.php atau halaman lainnya
                            header("Location: ../admin/admin.php");
                            exit();
                        } else {
                            // Tingkatan pengguna tidak valid, mungkin perlakukan sebagai kesalahan atau arahkan ke halaman default
